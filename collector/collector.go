...
func loadYamlMetricsConfig(_customMetrics string, metrics *Metrics) error {
	yamlBytes, err := os.ReadFile(_customMetrics)
	if err != nil {
		return fmt.Errorf("cannot read the metrics config %s: %w", _customMetrics, err)
	}
	if err := yaml.Unmarshal(yamlBytes, metrics); err != nil {
		return fmt.Errorf("cannot unmarshal the metrics config %s: %w", _customMetrics, err)
	}
	return nil
}
